...
class Aplicacion():
    def __init__(self):
        self.raiz = Tk()
        self.raiz.title("Gestión de tablas")
        self.raiz.geometry('1500x700')

        # Declara variables de control

        self.ip = StringVar()
        self.tabla = StringVar(value='t')
        self.community = StringVar()

        # Carga imagen para asociar a widget Label()

        #portatil = PhotoImage(file='portatil.png')

        # Declara widgets de la ventana
        # Se incluye el widget de tipo Button 'Next' que utiliza
        # la opción 'command' para validar datos

        #self.imagen1 = ttk.Label(self.raiz, image=portatil,
        #                         anchor="center")
        self.etiq1 = ttk.Label(self.raiz, text="Introduzca la ip del dispositivo que desea gestionar:")
        self.box_ip = ttk.Entry(self.raiz, textvariable=self.ip,
                              width=80)
        self.etiq2 = ttk.Label(self.raiz, text="¿Qué tabla desea ver?")
        self.obj1 = ttk.Radiobutton(self.raiz, text='tcpConnTable',
                                      variable=self.tabla, value='t')
        self.obj2 = ttk.Radiobutton(self.raiz, text='udpTable',
                                      variable=self.tabla, value='u')
        self.obj3 = ttk.Radiobutton(self.raiz, text='hrSWRunTable',
                                      variable=self.tabla, value='e')
        self.obj4 = ttk.Radiobutton(self.raiz, text='ipAddrTable',
                                    variable=self.tabla, value='i')
        self.obj5 = ttk.Radiobutton(self.raiz, text='ifTable',
                                    variable=self.tabla, value='if')
        self.obj6 = ttk.Radiobutton(self.raiz, text='ipRouteTable',
                                    variable=self.tabla, value='a')
        self.obj7 = ttk.Radiobutton(self.raiz, text='hrDeviceTable',
                                    variable=self.tabla, value='d')
        self.obj8 = ttk.Radiobutton(self.raiz, text='ipv4InterfaceTable',
                                    variable=self.tabla, value='i2')
        self.etiq3 = ttk.Label(self.raiz,
                               text="Introduzca la comunidad a la que pertenece el dispositivo que desea gestionar:")
        self.box_community = ttk.Entry(self.raiz, textvariable=self.community,
                              width=80)
        self.separ1 = ttk.Separator(self.raiz, orient=HORIZONTAL)

        self.button1 = Button(self.raiz, text="Siguiente", bg='snow4', fg='white',
                                 command=self.next)
        self.button2 = Button(self.raiz, text="Salir", bg='firebrick3', fg='white',
                                 command=quit)
        ...
        self.etiq1.pack(side=TOP, fill=BOTH, expand=True,
                        padx=10, pady=5)
        self.box_ip.pack(side=TOP, expand=True, anchor=W,
                        padx=20, pady=5)
        self.etiq2.pack(side=TOP, fill=BOTH, expand=True,
                        padx=10, pady=5)
        self.obj1.pack(side=TOP, fill=BOTH, expand=True,
                         padx=20, pady=5)
        self.obj2.pack(side=TOP, fill=BOTH, expand=True,
                         padx=20, pady=5)
        self.obj3.pack(side=TOP, fill=BOTH, expand=True,
                         padx=20, pady=5)
        self.obj4.pack(side=TOP, fill=BOTH,expand=True,
                         padx=20, pady=5)
        self.obj6.pack(side=TOP, fill=BOTH, expand=True,
                         padx=20, pady=5)
        self.obj5.pack(side=TOP, fill=BOTH, expand=True,
                       padx=20, pady=5)
        self.obj7.pack(side=TOP, fill=BOTH, expand=True,
                       padx=20, pady=5)
        self.obj8.pack(side=TOP, fill=BOTH, expand=True,
                       padx=20, pady=5)

        self.etiq3.pack(side=TOP, fill=BOTH, expand=True,
                        padx=10, pady=5)
        self.box_community.pack(side=TOP, expand=True, anchor=W,
                       padx=20, pady=5)
    ...
    def next(self):
        ...
        total = 0
        try:
            self.ip_u = str(self.ip.get())
            tabla = str(self.tabla.get())
            self.community_u = str(self.community.get())
        except:
            error_dato = True
        if not error_dato:
            if tabla == 't':
                self.gest_tcp(self.ip_u, self.community_u)
            if tabla == 'u':
                self.gest_udp(self.ip_u, self.community_u)
            if tabla == 'i':
                self.gest_ip(self.ip_u, self.community_u)
            if tabla == 'e':
                self.gest_host(self.ip_u, self.community_u)
            if tabla == 'a':
                self.gest_ip2(self.ip_u, self.community_u)
            if tabla == 'if':
                self.gest_if(self.ip_u, self.community_u)
            if tabla == 'd':
                self.gest_host2(self.ip_u, self.community_u)
            if tabla == 'i2':
                self.gest_ip3(self.ip_u, self.community_u)
        else:
            print("Error en los datos introducidos")

    def gest_tcp(self, ip, community):
    ...
    def gest_udp(self, ip, community):
    ...
    def gest_if(self, ip, community):
        ...
        self.window4.button2 = Button(self.window4, text="Salir", bg='firebrick1', fg='white',
    ...
    def gest_ip2(self, ip, community):
        ...
        self.window5.button2 = Button(self.window5, text="Salir", bg='firebrick1', fg='white',
    ...
    def fun_ip2(self):
        ...
        if not error_dato:
            ...
            print("Error en los datos introducidos o no seleccionada ninguna columna")
    def gest_host(self, ip, community):
        ...
        self.window6.button2 = Button(self.window6, text="Salir", bg='firebrick1', fg='white',
    ...
    def gest_host2(self, ip, community):
        ...
        self.window7.button2 = Button(self.window7, text="Salir", bg='firebrick1', fg='white',
    ...
    def gest_ip3(self, ip, community):
        self.window8 = Toplevel()
        self.window8.geometry('1500x700')
        self.window8.title("Tablas del grupo tcp")
        self.window8.col = ['1.3.6.1.2.1.4.28.1.1', '1.3.6.1.2.1.4.28.1.2', '1.3.6.1.2.1.4.28.1.3',
               '1.3.6.1.2.1.4.28.1.4']
        self.window8.col_nomb = ['ipv4InterfaceIfIndex', '	ipv4InterfaceReasmMaxSize', 'ipv4InterfaceEnableStatus', 'ipv4InterfaceRetransmitTime']
        self.window8.col1 = BooleanVar()
        self.window8.col2 = BooleanVar()
        self.window8.col3 = BooleanVar()
        self.window8.col4 = BooleanVar()
        self.window8.col5 = BooleanVar()
        self.window8.etq1 = ttk.Label(self.window8, text="Seleccione las columnas que desea ver:")
        self.window8.state = ttk.Checkbutton(self.window8, text='ipv4InterfaceIfIndex',
                                      variable=self.window8.col1,
                                      onvalue=True, offvalue=False)
        self.window8.localaddress = ttk.Checkbutton(self.window8, text='ipv4InterfaceReasmMaxSize',
                                      variable=self.window8.col2,
                                      onvalue=True, offvalue=False)
        self.window8.localport = ttk.Checkbutton(self.window8, text='ipv4InterfaceEnableStatus',
                                      variable=self.window8.col3,
                                      onvalue=True, offvalue=False)
        self.window8.remoteaddress = ttk.Checkbutton(self.window8, text='ipv4InterfaceRetransmitTime',
                                      variable=self.window8.col4,
                                      onvalue=True, offvalue=False)

        self.window8.separ1 = ttk.Separator(self.window8, orient=HORIZONTAL)

        self.window8.button1 = Button(self.window8, text="Mostrar tabla", bg='snow4', fg='white', command=self.fun_ip3)
        self.window8.button2 = Button(self.window8, text="Salir", bg='firebrick3', fg='white',
                              command=self.window8.destroy)
        self.window8.etq1.pack(side=TOP, fill=BOTH, expand=False,
                        padx=10, pady=5)
        self.window8.state.pack(side=TOP, fill=BOTH, expand=False,
                       padx=10, pady=5)
        self.window8.localaddress.pack(side=TOP, fill=BOTH, expand=False,
                       padx=10, pady=5)
        self.window8.localport.pack(side=TOP, fill=BOTH, expand=False,
                       padx=10, pady=5)
        self.window8.remoteaddress.pack(side=TOP, fill=BOTH, expand=False,
                       padx=10, pady=5)
        self.window8.separ1.pack(side=TOP, fill=BOTH, expand=True,
                         padx=5, pady=5)
        self.window8.button1.pack(side=LEFT, fill=BOTH, expand=True, padx=10, pady=10)
        self.window8.button2.pack(side=RIGHT, fill=BOTH, expand=True, padx=10, pady=10)

        self.window8.grab_set()

        self.raiz.wait_window(self.window8)

    def fun_ip3(self):
        error_dato = False
        if self.window8.col1.get() == False and self.window8.col2.get() == False and self.window8.col3.get() == False and self.window8.col4.get() == False:
            error_dato = True
        if not error_dato:
            list_col = []
            lista_col_nomb = []
            if (self.window8.col1.get() == True):
                list_col.append(self.window8.col[0])
                lista_col_nomb.append(self.window8.col_nomb[0])
            if (self.window8.col2.get() == True):
                list_col.append(self.window8.col[1])
                lista_col_nomb.append(self.window8.col_nomb[1])
            if (self.window8.col3.get() == True):
                list_col.append(self.window8.col[2])
                lista_col_nomb.append(self.window8.col_nomb[2])
            if (self.window8.col4.get() == True):
                list_col.append(self.window8.col[3])
                lista_col_nomb.append(self.window8.col_nomb[3])
    ...
    def obtenertabla(self, ip, columnas, community, col_nomb):
        ...
        self.buttonset = ttk.Button(self.tablas, text="Modificar tabla", width = 30)
